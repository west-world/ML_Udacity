...
'''
model2 = Sequential()
model2.add(Conv2D(filters=32, kernel_size=3, strides=2, padding='same',
    activation='relu', input_shape=(128, 128, 3)))
model2.summary()

model3 = Sequential()
model3.add(MaxPooling2D(pool_size=2,strides=2,input_shape=(100,100,15)))
model3.summary()
...
print('''\n\n                     Basic Loose laws of CNN \n
        ...
        \n\n''')
model4 = Sequential()
model4.add(Convolution2D(filters = 16,kernel_size=2,strides=1,padding='same',activation='relu',input_shape=(32,32,3)))
model4.add(MaxPooling2D(pool_size=2))
model4.add((Convolution2D(filters=32,kernel_size=2,strides=1,padding='same',activation='relu')))
model4.add(MaxPooling2D(pool_size=2))
model4.add(Convolution2D(filters=64,kernel_size=2,strides=1,padding='same',activation='relu'))
model4.add(MaxPooling2D(pool_size=2))
model4.add(Flatten())
...
print('''\n
        \tSteps for a Normal Dense Network(Non-CNN)\n
         1.  Start with a  model that is Sequntial()
                Split data into training, test set  and One Hot encode Labels.
         2.  First Flatten your input using Flatten , if its a multi-dimensional input(like an image)
         3.  Next add a Dense layer of 'x' number of layers with an activation function.
         4.  Next add a Dropout Layer with starting value of ~0.2
         5.  Repeat steps 3&4 for ow many ever hidden layers are needed.
         6.  Final Layer should be an output layer,with shape = unique number of labels, 
              and activation function as 'softmax' \n
         7.  Above steps finalize Model Architechture Design. Next is to compile and add a loss function.
         8.  Use model.compile to add a loss function and optimizer, with a target metric
         9.  Next is to Train the Model by doing the below
         10. Use ModelCheckPoint, to save the best weights only and resuse
         11. Use Model.Fit which adds in callbacks, Batch_Size, num of epochs and passes in training and validation sets
         12. Once the Model is trained, call the .load_weights to load the same file in Model CheckPoint
         13. Now call Model.Evaluate on the Test set with the loaded weights to test accuracy\n''')
regular_model = Sequential()
regular_model.add(Flatten(input_shape=x_train.shape[1:]))
regular_model.add(Dense(1000,activation='relu'))
regular_model.add(Dropout(0.2))
regular_model.add(Dense(512,activation='relu'))
regular_model.add(Dropout(0.2))
regular_model.add(Dense(num_classes,activation='softmax'))
regular_model.summary()
regular_model.compile(loss='categorical_crossentropy',optimizer='rmsprop',metrics=['accuracy'])
checkpointer = ModelCheckpoint(filepath='MLP.weights.best.hdf5',verbose=1,save_best_only=True)
hist = regular_model.fit(x_train,y_train,batch_size=128,epochs=20,verbose=2,callbacks=[checkpointer],validation_data=(x_valid,y_valid),shuffle=True)

regular_model.load_weights('MLP.weights.best.hdf5')
score = regular_model.evaluate(x_test,y_test,verbose=0)
print('\n Accuracy by Regular MLP Model = {}'.format(score[1]))
