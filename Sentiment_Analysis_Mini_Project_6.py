...
class SentimentNetwork:
    def __init__(self, reviews, labels, hidden_nodes=10,min_count=100,polarity_cutoff = 0.5,learning_rate=0.1):
    ...
    def calculate_pos_neg_ratio(self,reviews,labels):
        ...
        for review_num in range(0,len(reviews)):
            for word in reviews[review_num].split(" "):
                ...
                if(labels[review_num].lower() == 'positive'):
                    positive_counter[word]+=1
                else:
                    negative_counter[word]+=1
        for word in self.total_counts:
            if(self.total_counts[word]>50):
                self.pos_neg_ratio[word] = np.log(float(positive_counter[word]) / (negative_counter[word] + 1))
    ...
    def pre_process_data(self, reviews, labels,min_count,polarity_cutoff):
        ...
        self.calculate_pos_neg_ratio(reviews,labels)
        review_vocab = set()
        for review in reviews:
            for word in review.split(" "):
                if(self.total_counts[word] > min_count and abs(self.pos_neg_ratio[word]) > polarity_cutoff):
